...
[ ! -e googletest-release-$GTEST_VERSION ] && \
    ...
    tar -xvzf $GTEST_GZ --strip-components=2 -C googletest
...
[ ! -e libuv-$LIBUV_VERSION ] && \
    ...
    tar -xvzf $LIBUV_GZ --strip-components=2 -C libuv

[ ! -e uWebSockets ] && \
    wget http://github.com/dyu/uWebSockets/archive/$UWS_BRANCH.tar.gz -o $UWS_GZ && \
    tar -xvzf $UWS_GZ --strip-components=2 -C uWebSockets
